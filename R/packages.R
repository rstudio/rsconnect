...
#'
#' @description Deployed applications can depend on any package available on
...
#' using the [appDependencies()] function. The list of dependencies is sent to
#' the server along with the application source code and these dependencies are
#' then installed alongside the application.
#'
#' Note that the `Suggests` dependencies of packages are not automatically
#' included in the list of dependent packages. See the *Note* section of the
#' documentation of the [appDependencies()] function for details on how to force
...
#'   [devtools::install_github()] function from the \pkg{devtools} package.
#'
#'   This works because `install_github` records the exact Github commit that
...
#'   latest version of `devtools` from Github. You can do this as follows:
#'
#' \preformatted{
#'   library(devtools)
