...
test_that("can infer mode for API with plumber.R", {
  ...
  expect_equal(inferAppMode(dir, paths), list(appMode = "api", primaryFile = "plumber.R"))
})

test_that("can infer mode for API with entrypoint.R", {
  dir <- local_temp_app(list("entrypoint.R" = ""))
  paths <- list.files(dir)
  expect_equal(inferAppMode(dir, paths), list(appMode = "api", primaryFile = "entrypoint.R"))
})

test_that("can infer mode for API with _server.yml", {
  dir <- local_temp_app(list("_server.yml" = ""))
  paths <- list.files(dir)
  expect_equal(inferAppMode(dir, paths), list(appMode = "api", primaryFile = "_server.yml"))
})

test_that("can infer mode for shiny apps with app.R", {
  dir <- local_temp_app(list("app.R" = ""))
  paths <- list.files(dir)
  expect_equal(inferAppMode(dir, paths), list(appMode = "shiny", primaryFile = "app.R"))
})

test_that("can infer mode for shiny apps with server.R", {
  dir <- local_temp_app(list("server.R" = ""))
  paths <- list.files(dir)
  expect_equal(inferAppMode(dir, paths), list(appMode = "shiny", primaryFile = "server.R"))
})

test_that("can infer mode for static rmd", {
  dir <- local_temp_app(list("foo.Rmd" = ""))
  paths <- list.files(dir)
  expect_equal(inferAppMode(dir, paths), list(appMode = "rmd-static", primaryFile = "foo.Rmd"))
})

test_that("can infer mode for rmd as static quarto with guidance", {
  dir <- local_temp_app(list("foo.Rmd" = ""))
  paths <- list.files(dir)
  expect_equal(inferAppMode(dir, paths, usesQuarto = TRUE), list(appMode = "quarto-static", primaryFile = "foo.Rmd"))
})
...
test_that("can infer mode for rmd as shiny quarto with guidance", {
  ...
  dir <- local_temp_app(list("foo.Rmd" = ""))
  paths <- list.files(dir)
  expect_equal(inferAppMode(dir, paths, isShinyappsServer = TRUE), list(appMode = "rmd-shiny", primaryFile = "foo.Rmd"))
})

test_that("can infer mode for static quarto", {
  dir <- local_temp_app(list("foo.qmd" = ""))
  paths <- list.files(dir)
  expect_equal(inferAppMode(dir, paths), list(appMode = "quarto-static", primaryFile = "foo.qmd"))
  ...
  paths <- list.files(dir)
  expect_equal(inferAppMode(dir, paths), list(appMode = "quarto-static", primaryFile = "foo.rmd"))

  dir <- local_temp_app(list("_quarto.yml" = "", "foo.r" = ""))
  paths <- list.files(dir)
  expect_equal(inferAppMode(dir, paths), list(appMode = "quarto-static", primaryFile = "foo.r"))
...
test_that("can infer mode for shiny rmd docs", {
  ...
  paths <- list.files(dir)
  expect_equal(inferAppMode(dir, paths), list(appMode = "rmd-shiny", primaryFile = "index.Rmd"))
...
test_that("can infer mode for shiny qmd docs", {
  ...
  paths <- list.files(dir)
  expect_equal(inferAppMode(dir, paths), list(appMode = "quarto-shiny", primaryFile = "index.Qmd"))

  # Can force Rmd to use quarto
  dir <- local_temp_app(list("index.Rmd" = yaml_runtime("shiny")))
  paths <- list.files(dir)
  expect_equal(inferAppMode(dir, paths, usesQuarto = TRUE), list(appMode = "quarto-shiny", primaryFile = "index.Rmd"))
...
test_that("otherwise, fallsback to static deploy", {
  ...
  paths <- list.files(dir)
  expect_equal(inferAppMode(dir, paths), list(appMode = "static", primaryFile = NULL))
})
