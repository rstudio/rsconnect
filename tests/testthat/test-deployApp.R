...
test_that("applicationDeleted() errors or prompts as needed", {
  ...
  addTestAccount("a", "s")
  dir <- local_temp_app()
  addTestDeployment(dir, appName = "name", account = "a", server = "s")
  target <- createDeploymentTarget("name", "title", "id", NULL, "a", "a", "s")
  client <- list(createApplication = function(...) NULL)

  expect_snapshot(applicationDeleted(client, target, dir), error = TRUE)
  expect_length(dir(dir, recursive = TRUE), 1)

  simulate_user_input(2)
  expect_snapshot(. <- applicationDeleted(client, target, dir))
  expect_length(dir(dir, recursive = TRUE), 0)
})
